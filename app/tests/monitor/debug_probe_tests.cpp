...
void debug_probe_tests::
    runtime_bridge_replacement_listener_receives_fresh_bootstrap() {
#if defined(NDEBUG)
    QSKIP(
        "Debug monitor broadcasting is intentionally disabled in release "
        "builds."
    ...
    runtime_bridge runtime(
        runtime_bridge::runtime_options {
            .enabled = false,
            .requested_endpoint = QStringLiteral("yodau_probe_reconnect"),
        }
    );
    if (!runtime.set_enabled(true)) {
        QSKIP("QLocalServer::listen is unavailable in this environment.");
    }
    runtime.set_inventory(3, 2, 1, 4, 1);

    QLocalSocket first_socket;
    first_socket.connectToServer(runtime.endpoint_path());
    QVERIFY(first_socket.waitForConnected(1000));
    const QVector<QJsonObject> first_messages
        = read_protocol_messages(first_socket, 4, 2000);
    QVERIFY(!find_message_by_family(first_messages, QStringLiteral("hello"))
                 .isEmpty());

    QLocalSocket replacement_socket;
    replacement_socket.connectToServer(runtime.endpoint_path());
    QVERIFY(replacement_socket.waitForConnected(1000));
    const QVector<QJsonObject> replacement_messages
        = read_protocol_messages(replacement_socket, 4, 2000);

    QVERIFY(
        !find_message_by_family(replacement_messages, QStringLiteral("hello"))
             .isEmpty()
    );
    QVERIFY(!find_message_by_family(
                 replacement_messages, QStringLiteral("capabilities")
    )
                 .isEmpty());
    const QJsonObject snapshot_message = find_message_by_family(
        replacement_messages, QStringLiteral("snapshot")
    );
    QVERIFY(!snapshot_message.isEmpty());
    QCOMPARE(
        snapshot_message.value(QStringLiteral("snapshot"))
            .toObject()
...
void debug_probe_tests::runtime_bridge_refuses_to_steal_live_endpoint() {
#if defined(NDEBUG)
    QSKIP(
        "Debug monitor broadcasting is intentionally disabled in release "
        "builds."
...
#else
    runtime_bridge owner(
        runtime_bridge::runtime_options {
            .enabled = false,
            .requested_endpoint = QStringLiteral("yodau_probe_owned"),
        }
    );
    if (!owner.set_enabled(true)) {
        QSKIP("QLocalServer::listen is unavailable in this environment.");
    }

    runtime_bridge contender(
        runtime_bridge::runtime_options {
            .enabled = false,
            .requested_endpoint = QStringLiteral("yodau_probe_owned"),
        }
    );
    QVERIFY(!contender.set_enabled(true));
    QVERIFY(owner.is_enabled());
    QVERIFY(!contender.is_enabled());
#endif
}

void debug_probe_tests::runtime_bridge_endpoint_is_owner_only() {
#if defined(NDEBUG) || !defined(Q_OS_UNIX)
    QSKIP("Unix debug endpoint permissions are tested in debug builds.");
#else
    runtime_bridge runtime(
        runtime_bridge::runtime_options {
            .enabled = false,
            .requested_endpoint = QStringLiteral("yodau_probe_permissions"),
        }
    );
    if (!runtime.set_enabled(true)) {
        QSKIP("QLocalServer::listen is unavailable in this environment.");
    ...
    const QFileInfo endpoint(runtime.endpoint_path());
    QVERIFY(endpoint.exists());
    const QFileDevice::Permissions forbidden = QFileDevice::ReadGroup
        | QFileDevice::WriteGroup | QFileDevice::ExeGroup
        | QFileDevice::ReadOther | QFileDevice::WriteOther
        | QFileDevice::ExeOther;
    QVERIFY(
        (endpoint.permissions() & forbidden) == QFileDevice::Permissions {}
    );
    const QFileInfo directory(endpoint.absolutePath());
    QVERIFY(
        (directory.permissions() & forbidden) == QFileDevice::Permissions {}
    );
#endif
}
...
void debug_probe_tests::
    ...
    std::vector<yodau::core::event> events;
    ...
    motion_event.pos_pct = yodau::core::point { 12.5f, 44.0f };
