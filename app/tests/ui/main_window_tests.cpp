...
void main_window_tests::stream_catalog_tracks_seed_add_and_local_sources() {
    ...
    stream_catalog_state catalog_state;
    yodau::core::stream_manager stream_mgr;
    stream_mgr.set_local_stream_detector([] {
        std::vector<yodau::core::stream> detected;
        detected.emplace_back(
            "/tmp/fake-rear-camera", "Rear camera", "local", false
        );
        return detected;
    });
    ...
    QVERIFY(
        local_sources_combo->findData(QStringLiteral("/tmp/fake-rear-camera"))
        >= 0
    );
}
...
void main_window_tests::
    stream_cell_maps_rotated_fitted_coordinates_canonically() {
    stream_cell cell(QStringLiteral("cam-transform"));
    cell.resize(400, 400);
    cell.set_active(true);
    cell.set_keep_frame_aspect_ratio(true);
    cell.set_quarter_turns(1);
    cell.set_drawing_enabled(true);
    cell.set_repaint_interval_ms(1);
    cell.show();

    QSignalSpy layout_spy(&cell, &stream_cell::layout_geometry_changed);
    QImage frame(QSize(200, 100), QImage::Format_ARGB32_Premultiplied);
    frame.fill(Qt::black);
    QVERIFY(
        QMetaObject::invokeMethod(
            &cell, "on_frame_changed", Qt::DirectConnection,
    ...
    QCOMPARE(cell.source_frame_aspect(), 2.0);
    QCOMPARE(cell.effective_display_aspect(), 0.5);
    QVERIFY(!layout_spy.isEmpty());
    ...
    QCOMPARE(layout_spy.count(), 0);

    // A 90-degree clockwise turn maps canonical (20, 30) to display
    // percentage (70, 20). The 2:1 source is fitted into a centered 200x400
    // rectangle, so this click lands at (240, 80).
    QTest::mouseClick(&cell, Qt::LeftButton, Qt::NoModifier, QPoint(240, 80));
    // A click in the left pillarbox is clamped to the display-frame edge
    // before the inverse rotation, producing canonical (20, 100).
    QTest::mouseClick(&cell, Qt::LeftButton, Qt::NoModifier, QPoint(0, 80));

    const auto points = cell.draft_points_pct();
    QCOMPARE(points.size(), size_t(2));
    QVERIFY(
        main_window_tests_support::point_close(
            points.at(0), QPointF(20.0, 30.0)
        )
    );
    QVERIFY(
        main_window_tests_support::point_close(
            points.at(1), QPointF(20.0, 100.0)
        )
    );
}
...
void main_window_tests::
    ...
    const auto exported = yodau::core::load_line_configuration_file(
    ...
    const auto exported_from_data = yodau::core::decode_line_configuration_json(
        std::string_view(
...
void main_window_tests::window_state_store_rejects_invalid_payload() {
    QTemporaryDir temporary_directory;
    QVERIFY(temporary_directory.isValid());
    QSettings settings(
        temporary_directory.filePath(QStringLiteral("desktop-state.ini")),
    ...
    QVERIFY(!error.isEmpty());
    QVERIFY2(
        yodau::shell::save_main_window_state(window, settings, &error),
        qPrintable(error)
    );
}

void main_window_tests::grid_view_uses_packing_layout_plans() {
    grid_view view;
    view.resize(960, 420);
    view.show();
    view.add_stream(QStringLiteral("camera-d"));
    view.add_stream(QStringLiteral("camera-a"));
    view.add_stream(QStringLiteral("camera-c"));
    view.add_stream(QStringLiteral("camera-b"));

    QTRY_COMPARE(view.layout_row_count(), 2);
    QTRY_COMPARE(view.layout_column_count(), 2);
    QCOMPARE(view.layout_cell_count(), 4);
    QCOMPARE(
        view.stream_names(),
        QStringList(
            { QStringLiteral("camera-a"), QStringLiteral("camera-b"),
              QStringLiteral("camera-c"), QStringLiteral("camera-d") }
        )
    );

    view.resize(960, 210);
    QTRY_COMPARE(view.layout_row_count(), 1);
    QTRY_COMPARE(view.layout_column_count(), 4);
    QCOMPARE(view.layout_cell_count(), 4);

    view.remove_stream(QStringLiteral("camera-b"));
    QTRY_COMPARE(view.layout_cell_count(), 3);

    view.set_preserve_stream_order(true);
    QVERIFY(view.preserves_stream_order());
    QTRY_COMPARE(view.layout_cell_count(), 3);

    view.set_scroll_direction(grid_view::scroll_direction::vertical);
    QCOMPARE(
        view.current_scroll_direction(), grid_view::scroll_direction::vertical
    );
    QTRY_COMPARE(view.layout_cell_count(), 3);

    view.set_sizing_mode(grid_view::sizing_mode::fixed);
    QCOMPARE(view.current_sizing_mode(), grid_view::sizing_mode::fixed);
    QTRY_COMPARE(view.layout_cell_count(), 3);
    for (const QString& stream_name : view.stream_names()) {
        const stream_cell* cell = view.peek_stream_cell(stream_name);
        QVERIFY(cell != nullptr);
        QVERIFY(cell->has_manual_short_side());
        QVERIFY(cell->manual_short_side() >= 96.0);
    }
}

void main_window_tests::grid_view_handles_session_sizes_and_viewport_changes() {
    grid_view view;
    view.resize(960, 210);
    view.show();
    view.add_stream(QStringLiteral("camera-a"));
    ...
    auto* scroll = view.findChild<QScrollArea*>(
        QStringLiteral("stream_grid_scroll_area")
    );
    QVERIFY(scroll != nullptr);
    QTRY_VERIFY(scroll->horizontalScrollBar()->isVisible());
    QTRY_VERIFY(([&view, scroll]() {
        for (const QString& stream_name : view.stream_names()) {
            const stream_cell* cell = view.peek_stream_cell(stream_name);
            if (cell == nullptr
                || cell->geometry().bottom() >= scroll->viewport()->height()) {
                return false;
            }
        }
        return true;
    ...
    view.set_sizing_mode(grid_view::sizing_mode::fixed);
    stream_cell* first = view.peek_stream_cell(QStringLiteral("camera-a"));
    QVERIFY(first != nullptr);
    first->initialize_manual_short_side(222.0);
    view.set_sizing_mode(grid_view::sizing_mode::adaptive);
    view.hide();

    bool restore_question_seen = false;
    QTimer::singleShot(0, [&restore_question_seen]() {
        for (QWidget* widget : QApplication::topLevelWidgets()) {
            auto* question = qobject_cast<QMessageBox*>(widget);
            if (question != nullptr) {
                restore_question_seen = true;
    ...
    view.set_sizing_mode(grid_view::sizing_mode::fixed);
    QVERIFY(restore_question_seen);
    QCOMPARE(first->manual_short_side(), 222.0);

    view.show();
    first->initialize_manual_short_side(600.0);
    first->set_quarter_turns(1);
    QTRY_VERIFY(scroll->verticalScrollBar()->isVisible());
    QTRY_VERIFY(
        std::min(first->width(), first->height())
        >= qRound(first->manual_short_side())
    );
}

void main_window_tests::grid_preferences_persist_through_main_window() {
    QTemporaryDir temporary_directory;
    QVERIFY(temporary_directory.isValid());
    const QByteArray previous_config_home = qgetenv("XDG_CONFIG_HOME");
    const auto restore_config_home = qScopeGuard([previous_config_home]() {
    ...
    {
        main_window window;
        auto* direction = window.findChild<QComboBox*>(
            QStringLiteral("settings_grid_scroll_direction_combo")
        );
        auto* sizing = window.findChild<QComboBox*>(
            QStringLiteral("settings_grid_sizing_mode_combo")
        );
        auto* preserve = window.findChild<QCheckBox*>(
            QStringLiteral("settings_grid_preserve_order_checkbox")
        );
        QVERIFY(direction != nullptr);
        QVERIFY(sizing != nullptr);
        QVERIFY(preserve != nullptr);
        direction->setCurrentIndex(
            direction->findData(QStringLiteral("vertical"))
        );
        sizing->setCurrentIndex(sizing->findData(QStringLiteral("fixed")));
...

    {
        ...
        const auto* grid = restored.findChild<grid_view*>();
        QVERIFY(grid != nullptr);
        QCOMPARE(
            grid->current_scroll_direction(),
            grid_view::scroll_direction::vertical
        );
        QVERIFY(grid->preserves_stream_order());
        QCOMPARE(grid->current_sizing_mode(), grid_view::sizing_mode::fixed);
    }
}
