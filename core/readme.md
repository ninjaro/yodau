...
The core runtime now supports two rendering ownership modes:

* `core_only` - used by the CLI. OpenCV capture, detection, and core-owned preview routing now run in the core library through `processing_preview_router`, which renders line, event, and algorithm overlays before optional virtual-camera publication.
* `app_only` - used by the Qt app. Qt decodes and draws frames, while the core library only performs detection and emits events.

In CLI `core_only` mode, `yodau` does not open preview windows. Instead, on Linux it tries to publish rendered frames into a V4L2 output device so the stream can be opened externally, for example with `ffplay /dev/yodau-video0`, and can appear in webcam pickers if the system exposes that loopback device to other applications.
That core preview path now consumes `processing_result.overlays` in addition
to the generic stream-line and event markers.

The CLI now also keeps an in-memory log history with two output modes:

* `release` - concise operator-readable entries with severity, stream, event,
  optional line or region name, and short action text. Debug noise such as
  motion spam is hidden.
* `debug` - richer diagnostic entries with scope, subsystem, stream,
  line/region, algorithm, event, and details.

```bash
yodau> set-log-mode debug
yodau> show-log --limit=50 --severity=warning --stream=cam0 --event=tripwire
yodau> show-log --line=north --algorithm=contour_mask
yodau> clear-log
```

* `show-log` reads the retained CLI log history and can filter by severity,
  stream, line/region, event, algorithm, and subsystem.
...
The core now exposes four concrete processing algorithms:
...
* per-stream selection state, cached algorithm instances, and latest
  `processing_result` values now live in the runtime's
  `processing_session_store` helper rather than being embedded directly in the
  execution path
...
The repo now also exposes a Stage 0 benchmark harness for core algorithm
...
The repo now also includes a report generator plus the first recorded synthetic
comparison run:

* `scripts/build_benchmark_report.py`
* `benchmarks/reports/2026-04-02_dev_laptop_cpu_stage1_synthetic_report.json`
* `benchmarks/reports/2026-04-02_dev_laptop_cpu_stage1_synthetic_report.md`

That recorded report keeps `motion_baseline` as the explicit
`reference_algorithm_id` until a later benchmark review changes the rule.
...
* The Linux core runtime expects an existing virtual camera output device.
  By default it prefers `/dev/yodau-video0`, `/dev/yodau-video1`, ... and then compatible loopback-style `/dev/videoN` outputs.
* Set `YODAU_VCAM_DEVICE=/dev/some-device` to pin the core runtime to a specific virtual camera device.
* `list-virtual-cameras` prints the core stream-to-device bindings, readiness, and the last device error if no compatible sink is currently available.
...
The current core `line` type is geometry-only. It does not store visual
width, effective string length, damping/response, or other richer string-like
semantics. Those fields now have a separate core `line_profile` foothold,
and app save/apply flows plus CLI line creation now populate that profile.
The core line contract still stays geometry-only, while core-only preview
...
* Optional profile flags update the separate core `line_profile` record
