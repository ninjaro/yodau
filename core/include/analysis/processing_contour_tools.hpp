...
std::optional<point>
contour_center_pct(const std::vector<cv::Point>& contour, const cv::Size& size);
...
accepted_contours_by_area(const cv::Mat& binary_mask, double min_area_px2);

std::vector<processing_contour_candidate> contour_candidates_by_area(
    const cv::Mat& binary_mask, const cv::Size& frame_size, double min_area_px2
);
...
std::vector<processing_candidate> processing_candidates_from_contours(
    ...
    const std::optional<std::string>& class_id = std::nullopt
);
