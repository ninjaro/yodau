...
public:
    ...
    using legacy_algorithm_factory = std::function<std::unique_ptr<processing_algorithm>(
...
private:
    processing_algorithm_settings resolved_algorithm_settings_for_stream_locked(
        const std::string& stream_name
    ) const;
