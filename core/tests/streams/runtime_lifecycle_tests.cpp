...
TEST(processing_runtime_test, hot_unplug_releases_per_stream_state) {
    ...
    manager.set_local_stream_detector([&camera_present] {
        ...
        if (camera_present) {
            detected.emplace_back(
                "/tmp/fake-video2", "Rear camera", "local", false
            );
        }
        return detected;
    });
    ASSERT_TRUE(manager.find_stream("Rear camera"));
    EXPECT_EQ(
        manager.detected_local_stream_names(),
        std::vector<std::string>({ "Rear camera" })
    );
    runtime.attach(manager);
    runtime.set_stream_processing_max_pixels("Rear camera", 640 * 480);
    ...
    ASSERT_TRUE(
        runtime.set_stream_algorithm("Rear camera", alternate_algorithm)
    );

    camera_present = false;
    manager.refresh_local_streams();

    EXPECT_FALSE(manager.find_stream("Rear camera"));
    EXPECT_TRUE(manager.detected_local_stream_names().empty());
    EXPECT_FALSE(
        runtime.stream_processing_max_pixels("Rear camera").has_value()
    );
    EXPECT_FALSE(runtime.stream_algorithm_overrides().contains("Rear camera"));
    manager.shutdown();
    runtime.detach();
}
