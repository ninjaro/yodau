...
namespace default_processing_hooks_support {
    ...
    std::string
    normalized_requested_algorithm_id(const std::string& algorithm_id) {
    ...
    processing_algorithm_registry::entry registry_entry(
        ...
        return processing_algorithm_registry::entry {
            ...
            .display_name
            = processing_algorithm_display_name(normalized_algorithm_id),
    ...
    const processing_algorithm_registry& registry_instance() {
        static const processing_algorithm_registry registry = [] {
            ...
            value.register_algorithm(registry_entry(contour_mask, [] {
                return make_contour_mask_algorithm();
            }));
            value.register_algorithm(registry_entry(centroid_track, [] {
                return make_centroid_track_algorithm();
            }));
            value.register_algorithm(registry_entry(hybrid_auto, [] {
...
std::string default_processing_algorithm_id() {
    ...
    return default_processing_algorithm_registry().contains(candidate)
        ? candidate
        : std::string {};
}
