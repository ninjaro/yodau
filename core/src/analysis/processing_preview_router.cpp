...
namespace processing_preview_router_support {
    ...
    cv::Scalar color_from_text(const std::string& text) {
        ...
        const double blue = 64.0 + static_cast<double>(hash_value & 0x7fU);
        const double green
            = 64.0 + static_cast<double>((hash_value >> 7U) & 0x7fU);
        const double red
            = 64.0 + static_cast<double>((hash_value >> 14U) & 0x7fU);
    ...
    void draw_line_overlays(cv::Mat& image, const stream& stream_value) {
        ...
        for (const auto& line_ptr_value : lines) {
            ...
            const auto profile
                = stream_value.find_line_profile(line_ptr_value->name);
            ...
            cv::polylines(
                image, polyline, closed, color, thickness, cv::LINE_AA
            );
    ...
    void draw_event_overlays(cv::Mat& image, const std::vector<event>& events) {
        for (const auto& event_value : events) {
            ...
            if (event_value.kind == event_kind::tripwire
                ...
                draw_label(
                    image, event_value.line_name, center + cv::Point(8, -8),
                    color
                );
            }
        }
    ...
    void
    draw_processing_overlays(cv::Mat& image, const processing_result& result) {
        for (const auto& overlay : result.overlays) {
            ...
            case processing_overlay_kind::point:
                if (overlay.anchor_pct.has_value()) {
                    ...
                    draw_label(
                        image, overlay.label, anchor + cv::Point(8, -8), color
                    );
                }
                break;
            ...
            case processing_overlay_kind::polygon:
                if (overlay.points_pct.size() >= 2) {
                    ...
                    const cv::Point label_anchor
                        = overlay.anchor_pct.has_value()
                        ? pct_to_pixel(*overlay.anchor_pct, image)
                        : points.front();
                    draw_label(
                        image, overlay.label, label_anchor + cv::Point(8, -8),
                        color
                    );
                }
                break;
...
processing_preview_router::processing_preview_router(
    const bool enable_virtual_camera, std::string virtual_camera_device
) {
    if (enable_virtual_camera) {
        preview_camera_value = std::make_unique<virtual_camera>(
            "yodau", std::move(virtual_camera_device)
        );
    }
}
...
void processing_preview_router::publish_processed_frame(
    ...
    const frame rendered
        = processing_preview_router_support::render_overlay_frame(
