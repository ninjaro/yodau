    ...
    public:
        ...
        processing_result process_frame(
            ...
            const int line_count
                = static_cast<int>(stream_value.lines_snapshot().size());
            ...
            if (has_previous) {
                const cv::Mat diff
                    = blurred_absdiff(previous_gray, gray, blur_kernel);

                cv::Mat probe_energy;
                cv::resize(
                    diff, probe_energy,
                    cv::Size(probe_grid_cols, probe_grid_rows), 0.0, 0.0,
                    cv::INTER_AREA
                );
                ...
                for (int row = 0; row < probe_energy.rows; ++row) {
                    for (int col = 0; col < probe_energy.cols; ++col) {
                        if (static_cast<int>(
                                probe_energy.at<std::uint8_t>(row, col)
                            )
                            >= diff_threshold) {
                            hot_cells += 1;
                        }
                ...
                const int total_cells
                    = std::max(1, probe_grid_cols * probe_grid_rows);
            ...
            const double elapsed_ms
                ...
                / 1000.0;
            const double updated_average_processing_ms
        ...
        static selection_decision select_algorithm(
            ...
            if (overload_avg_ms == 0
                || average_processing_ms
                    >= static_cast<double>(overload_avg_ms)) {
                return selection_decision {
                    .algorithm_id = spot_grid,
                    .reason
                    = overload_avg_ms == 0 ? "forced_load_guard" : "load_guard",
                    .scene_state = "load_guard",
                };
            }

            if (last_selected_algorithm == spot_grid && recover_avg_ms > 0
                && average_processing_ms
                    >= static_cast<double>(recover_avg_ms)) {
                return selection_decision {
                    .algorithm_id = spot_grid,
            ...
            if (line_count > 0 && last_selected_algorithm == motion_baseline
                && motion_permille > 0) {
                return selection_decision {
                    .algorithm_id = motion_baseline,
            ...
            if (line_count == 0 && last_selected_algorithm == centroid_track
                && motion_permille > 0) {
                return selection_decision {
                    .algorithm_id = centroid_track,
            ...
            return selection_decision {
                .algorithm_id = spot_grid,
                .reason = motion_permille <= calm_motion_permille
                    ? "calm_scene"
                    : "low_value_motion",
                .scene_state
                = motion_permille <= calm_motion_permille ? "calm" : "sparse",
            };
        }

        processing_algorithm*
        delegate_for_algorithm(const std::string& algorithm_id) {
